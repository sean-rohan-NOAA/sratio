...
get_data <- function(species_codes) {
  ...
  standard_tows_1998 <- RODBC::sqlQuery(channel = channel, query = "select hauljoin, net_measured, wire_length, start_time, performance, 
    ...
    and gear = 44
    and accessories = 15") |>
    dplyr::inner_join(anti_join(otto_key_1998, special_tows_1998) |>
  ...
  # Get Goddard (1997) and Somerton et al. (2002) data
  ...
  crab <- dplyr::bind_rows(crab, crab_2023, crab_1995_1998) |>
    dplyr::inner_join(dplyr::select(all_hauls, HAULJOIN, MATCHUP))

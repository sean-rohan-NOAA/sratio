...
ss_map_subarea1_all_years <- ggplot() +
  geom_sf(data = bssa_layers$akland) +
  geom_sf(data = bathy, linewidth = 0.1) +
  geom_sf(data = target_strata,
          mapping = aes(fill = STRATUM),
...
ss_map_subarea6_all_years <- ggplot() +
  geom_sf(data = bssa_layers$akland) +
  geom_sf(data = bathy, linewidth = 0.1) +
  geom_sf(data = target_strata,
          mapping = aes(fill = STRATUM),
...
plot_legend_all_years <- cowplot::get_legend(ss_map_subarea1_all_years)

ss_map_all_tows_all_years <- cowplot::plot_grid(
    ss_map_subarea1_all_years + 
      theme(
            plot.margin = unit(c(2, 2, 0, 2), units = "mm")
        ),
    ss_map_subarea6_all_years + 
      theme(legend.position = "none", 
            plot.margin = unit(c(0, 2, 2, 2), units = "mm")
            ),
    nrow = 2
  )

ragg::agg_png(here::here("analysis", "shelf_slope", "plots", "Shelf_slope_tow_locations_2023_2024.png"),  
              height = 6, width = 3, units = "in", res = 300)
print(ss_map_all_tows_all_years)
dev.off()

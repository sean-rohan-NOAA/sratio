...
sccal_bootstrap_path <- list.files(here::here("analysis", "shelf_slope", "output"), 
                                     recursive = TRUE, 
                                     pattern = "sccal_model_bootstrap_results_", 
                                     full.names = TRUE)

sratio_bootstrap_path <- list.files(here::here("analysis", "shelf_slope", "output"), 
                                   recursive = TRUE, 
...
pratio_samples <- readRDS(here::here("analysis", "shelf_slope", "output", "pratio_samples.rds")) |>
...
for(ii in 1:length(sp_codes)) {
  ...
  plot_ratios <- ggplot() +
    ...
    scale_y_continuous(name = expression(italic(S['L,PNE,83-112'])),
  ...
  png(filename = here::here("analysis", "shelf_slope", "plots", 
                            paste0(sp_codes[ii], "_sccal_vs_sratio.png")), 
  ...
  png(filename = here::here("analysis", "shelf_slope", "plots", 
                            paste0(sp_codes[ii], "_sccal_vs_sratio_observations.png")), 
      width = 70, height = 70, units = "mm", res = 300)
  print(ggplot() +
          geom_abline(intercept = 0, slope = 1) +
          geom_point(data = sccal_vs_sr, mapping = aes(x = obs_ratio_sccal, 
                                                y = obs_ratio_sr)) +
          scale_x_continuous(name = expression(italic(S['L,PNE,83-112']~(SCCAL)))) +
          scale_y_continuous(name = expression(italic(S['L,PNE,83-112']~(SR)))))
  dev.off()

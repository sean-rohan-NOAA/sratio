...
# Treatment levels
treatments <- factor(c(172, 44))

seed <- 19710
n_boot_draws <- 1000

dat <- readRDS(here::here("analysis", "shelf_slope", "output", "catch_at_length_ss.rds")) |>
...
for(jj in 1:length(sp_code)) {
  ...
  saveRDS(boot_samples, file = here::here("analysis", "shelf_slope", 
                                          "output", 
                                          sp_code[jj], 

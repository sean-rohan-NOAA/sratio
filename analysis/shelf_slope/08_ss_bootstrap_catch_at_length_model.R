...
cal_rmse_df <- read.csv(file = here::here("analysis", "shelf_slope", "output", "cal_model_rmse.csv"))
...
for(ii in 1:length(bootstrap_sample_path)) {
  ...
  saveRDS(bootstrap_df, file = here::here("analysis", "shelf_slope", "output", sp_code,  
                                          paste0("sccal_bootstrap_results_", sp_code, ".rds")))

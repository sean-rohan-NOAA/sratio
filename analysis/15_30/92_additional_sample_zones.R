...
sampled_stations <- dplyr::filter(map_layers$survey.grid,
                                  STATIONID %in% hauls_2024$STATIONID)
...
idw_results[[ii]]$plot + 
                   geom_sf(data = sampled_stations, fill = NA, color = "white") +
                   geom_sf_text(data = map_layers$survey.grid |>
                                  sf::st_centroid(),

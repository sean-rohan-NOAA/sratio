...
for(jj in 1:length(model_method)) {
  ...
  for(ii in 1:length(sp_codes)) {
    ...
    size_30 <- readRDS(here::here("analysis", "15_30", "output", "catch_at_length_1530.rds")) |>
      dplyr::filter(TREATMENT == 30, SPECIES_CODE == sp_codes[ii]) |>
      dplyr::ungroup()
    
    size_15 <- readRDS(here::here("analysis", "15_30", "output", "catch_at_length_1530.rds")) |>
    ...
    saveRDS(object = list(species_code = sp_codes[ii],
            ...
            file = here::here("analysis", "15_30", "output", paste0("model_performance_", model_method[jj], "_", sp_codes[ii], ".rds")))
